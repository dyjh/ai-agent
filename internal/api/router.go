...
func NewRouter(deps Dependencies) http.Handler {
	...
	securityHandler := handlers.NewSecurityHandler(deps)
	evalsHandler := handlers.NewEvalsHandler(deps)
	...
	r.Route("/v1", func(r chi.Router) {
		...
		r.Post("/runs/{run_id}/replay", evalsHandler.ReplayRun)
		r.Post("/runs/{run_id}/resume", runs.Resume)
		r.Post("/runs/{run_id}/cancel", runs.Cancel)
		r.Get("/replays/{replay_id}", evalsHandler.GetReplay)

		r.Get("/evals", evalsHandler.List)
		r.Post("/evals", evalsHandler.Create)
		r.Post("/evals/run", evalsHandler.Run)
		r.Get("/evals/runs", evalsHandler.ListRuns)
		r.Get("/evals/runs/{run_id}", evalsHandler.GetRun)
		r.Get("/evals/runs/{run_id}/report", evalsHandler.GetReport)
		r.Get("/evals/{case_id}", evalsHandler.Get)
		r.Patch("/evals/{case_id}", evalsHandler.Update)
		r.Delete("/evals/{case_id}", evalsHandler.Delete)

		r.Get("/memory/files", memory.ListFiles)
		r.Get("/memory/files/*", memory.GetFile)
		r.Post("/memory/search", memory.Search)
		r.Get("/memory/items", memory.ListItems)
		r.Post("/memory/items", memory.CreateItem)
		r.Get("/memory/items/{id}", memory.GetItem)
		r.Patch("/memory/items/{id}", memory.UpdateItem)
		r.Delete("/memory/items/{id}", memory.DeleteItem)
		r.Post("/memory/items/{id}/archive", memory.ArchiveItem)
		r.Post("/memory/items/{id}/restore", memory.RestoreItem)
		r.Post("/memory/review/extract", memory.ExtractReview)
		r.Get("/memory/review", memory.ListReview)
		r.Get("/memory/review/{review_id}", memory.GetReview)
		r.Post("/memory/review/{review_id}/approve", memory.ApproveReview)
		r.Post("/memory/review/{review_id}/reject", memory.RejectReview)
		...
		r.Get("/security/policy-profiles", securityHandler.ListPolicyProfiles)
		r.Get("/security/policy-profiles/{name}", securityHandler.GetPolicyProfile)
		r.Post("/security/policy-profiles/validate", securityHandler.ValidatePolicyProfile)
		r.Post("/security/secret-scan", securityHandler.SecretScan)
		r.Get("/security/network-policy", securityHandler.NetworkPolicy)
		r.Post("/security/network-policy/validate-url", securityHandler.ValidateURL)
		r.Get("/security/audit", securityHandler.Audit)
		r.Get("/security/audit/runs/{run_id}", securityHandler.AuditRun)
	})
